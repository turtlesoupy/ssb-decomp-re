...
void mvOpeningMarioMakeName(void)
{
...
#ifdef PORT
	{
		...
		if (port_ui_opening_name_hook(gobj, sMVOpeningMarioFiles[0], nFTKindMario)) return;
	}
#endif
...
void mvOpeningMarioMakePosedFighter(void)
{
...
#ifdef PORT
	{
		...
		desc.fkind = port_roster_opening_spawn_fkind(1, nFTKindMario);
	}
...
void mvOpeningMarioFuncStart(void)
{
	...
	{
		extern s32 port_roster_opening_spawn_fkind(s32, s32);
		gSCManagerBattleState->players[0].fkind = port_roster_opening_spawn_fkind(0, nFTKindMario);
	}
#else
