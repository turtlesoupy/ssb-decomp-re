...
void mvOpeningKirbyMakeName(void)
{
...
#ifdef PORT
	{
		...
		if (port_ui_opening_name_hook(gobj, sMVOpeningKirbyFiles[0], nFTKindKirby)) return;
	}
#endif
...
void mvOpeningKirbyFuncStart(void)
{
	...
	{
		extern s32 port_roster_opening_spawn_fkind(s32, s32);
		gSCManagerBattleState->players[0].fkind = port_roster_opening_spawn_fkind(0, nFTKindKirby);
	}
#else
