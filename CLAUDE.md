...
- **Merging upstream:** `git merge upstream/main`. `rerere` is enabled and has the content resolutions cached. Afterwards regenerate the machine-owned PORT halves of the FTMotionDesc tables and prove them: from the BattleShip root run `python3 tools/derive_ftmotion_port_tables.py`, then `python3 tools/check_ftdata_tables.py --version us` and `--version jp` (both must report 0 disagreements). Four paths are deliberately absent and will conflict as modify/delete — `.gitmodules` and the `tools/{splat,asm-processor,asm-differ}` gitlinks — because the port's CI checks out with `submodules: recursive` and does not need them. Keep them deleted.
